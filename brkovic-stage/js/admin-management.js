(function () {
  ...
  function labelFor(item, key) {
    return item?.title || labelFallbacks[item?.label] || item?.label || key;
  }
  ...
  function monthlyAddOnEntries(typeKey = selectedContractType()) {
    return MONTHLY_ADDON_KEYS
      .map((key) => [key, pricing?.monthlyServices?.[key]])
      .filter(([, item]) => serviceAvailableForType(item, typeKey));
  }
  ...
  function orderedEntries(groupKey) {
    ...
    (groupOrder[groupKey] || []).forEach((key) => {
    ...
    Object.entries(group).forEach(([key, item]) => {
      if (!seen.has(key)) entries.push([key, item]);
    });
  ...
  function sailingOnlyMarkup(key, item) {
    return `
      <label class="pricing-field pricing-field--toggle">
        <span>Только парусные</span>
        <input type="checkbox" data-group="monthlyServices" data-key="${escapeHtml(key)}" data-field="sailingOnly" ${item.sailingOnly ? 'checked' : ''} />
  ...
  function renderMonthlyAddOnPricingFields() {
    ...
    if (!node) return;
    const entries = MONTHLY_ADDON_KEYS.map((key) => [key, pricing?.monthlyServices?.[key]]).filter(([, item]) => item);
