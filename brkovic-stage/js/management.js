(function () {
  ...
  const PRICING_URL = new URL('../data/management-pricing.json?v=20260516-12', scriptUrl).href;
  ...
  function renderDynamicChoices() {
    ...
    if (monthlyNode) {
      const rows = orderedServiceEntries(monthlyServices, MONTHLY_ADDON_KEYS).map(([key, item]) => `
        <label class="management-service-line">
          <input type="checkbox" name="monthly" value="${key}" ${selectedMonthly.has(key) ? 'checked' : ''} />
          <span>
            <strong>${t(item.label, key)}</strong>
  ...
  function render() {
    ...
    if (summary) {
      ...
      const oneTimeLabels = result.oneTimeDetails
        .map(({ key, quantity }) => {
          const service = oneTimeServices[key];
          return service && service.enabled !== false ? `${t(service.label)} × ${quantity}` : '';
        })
        .filter(Boolean);
      const items = [
